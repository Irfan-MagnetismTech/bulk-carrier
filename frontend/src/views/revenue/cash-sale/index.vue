...
import useCashSale from "../../../composables/revenue/useCashSale";
...
const { cashSales, getCashSales, deleteCashSale, isLoading } = useCashSale();
...
watch(searchKey, newQuery => {
  getCashSales(props.page, columns, searchKey.value, table);
});
...
function print(saleInfo) {
  ...
  for(let index in materials) {
    ...
    if(fuel.material) {
      ...
      serviceInfo += `
        <tr>
            <td>${ fuel.service.name }</td>
            <td>${ Math.abs(fuel.quantity) }</td>
            <td>${ numberFormat(fuel.service_rate.rate) }</td>
            <td>${ numberFormat(fuel.amount) }</td>
        </tr>
        `
    }
  ...
  var newWindowContent = `
        <div id="printableArea" class="w-full">
                        <div>
                            ...
                            <div class="flex justify-between ml-2" style="overflow: hidden; margin: 10px auto;" id="date">
...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-2 sm:flex-row" v-once>
    <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Cash Sales</h2>
    <router-link :to="{ name: 'revenue.cash-sale.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61] rounded-lg active:bg-[#0F6B61] hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
  ...
  <div class="flex items-center justify-between mb-2 select-none">
    <span class="w-full text-xs font-medium text-gray-500 whitespace-no-wrap">Showing {{ cashSales?.from }}-{{ cashSales?.to }} of {{ cashSales?.total }}</span>
    <!-- Search -->
    ...
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap mb-8">
        ...
        <tr class="text-xs font-semibold tracking-wide text-center text-gray-500 uppercase bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
          ...
          <th class="px-4 py-3 w-16">Shift</th>
          <th class="px-4 py-3 w-32">Sold By</th>
          <th class="px-4 py-3 w-96">Materials</th>
          <th class="px-4 py-3 w-32">Total Amount</th>
          <th class="px-1 py-3 w-16">Card</th>
          <th class="px-4 py-3 text-center">Actions</th>
        </tr>
        </thead>
        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(cashSale,index) in (cashSales?.data ? cashSales?.data : cashSales)" :key="cashSale.id">
          <td class="px-4 py-3 text-sm">{{ (cashSales?.from) ? cashSales?.from + index : index + 1 }}</td>
          <td class="px-4 py-3 text-sm">{{ (cashSale.date_time) ? moment(cashSale.date_time).format('DD/MM/YYYY hh:mm A') : ""  }}</td>
          <td class="px-4 py-3 text-sm">{{ cashSale?.shift }}</td>
          <td class="px-4 py-3 text-sm">{{ cashSale?.entry_by?.name }}</td>
          <td class="px-4 py-3 text-sm">

            <template v-for="(stockable,index) in cashSale?.materials" :key="index">
              <span class="p-1 bg-gray-100 rounded-md m-1 inline-block" v-if="stockable.material != null">
                {{ stockable?.material?.name }}
              </span>
              <span class="p-1 bg-gray-100 rounded-md m-1 inline-block" v-if="stockable.service != null">
                {{ stockable?.service?.name }}
              </span>
            </template>


          </td>
          <td class="px-4 py-3 text-sm">{{ numberFormat(cashSale?.total_amount) }}</td>
          <td class="px-1 py-3 text-sm">
            <span v-if="cashSale?.payment_method == 'Card'">
          ...
          <td class="items-center justify-center px-4 py-3 space-x-2 text-sm text-gray-600">
            <action-button :action="'edit'" :to="{ name: 'revenue.cash-sale.edit', params: { cashSaleId: cashSale.id } }"></action-button>
            <action-button :action="'show'" :to="{ name: 'revenue.cash-sale.show', params: { cashSaleId: cashSale.id } }"></action-button>
            <action-button @click="print(cashSale)" :action="'print'"></action-button>
            <action-button @click="deleteCashSale(cashSale.id)" :action="'delete'"></action-button>
          </td>
        </tr>
        </tbody>
        <tfoot v-if="!(cashSales?.data ? cashSales?.data.length : cashSales?.length)" class="bg-white dark:bg-gray-800">
        <tr v-if="isLoading">
          <td colspan="8">Loading...</td>
        </tr>
        <tr v-else-if="!(cashSales?.data ? cashSales?.data.length : cashSales?.length)">
          <td colspan="8">No Cash Sale found.</td>
        </tr>
        </tfoot>
      </table>
    </div>
    <Paginate :data="cashSales" to="revenue.cash-sale.index" :page="page"></Paginate>
  </div>
</template>
