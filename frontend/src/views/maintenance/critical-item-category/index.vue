...
const { criticalItemCategories, getCriticalItemCategories, deleteCriticalItemCategory, isLoading, isTableLoading, errors } = useCriticalItemCategory();
...
function confirmDelete(id) {
  Swal.fire({
    title: 'Are you sure?',
    text: "You want to delete this critical Item Category!",
    icon: 'warning',
...
let filterOptions = ref( {
  ...
  "filter_options": [
    {
      "rel_type": null,
      "relation_name": "mntCriticalFunction",
...
onMounted(() => {
  watchPostEffect(() => {
    if(currentPage.value == props.page && currentPage.value != 1) {
      ...
      router.push({ name: 'mnt.critical-item-categories.index', query: { page: filterOptions.value.page } });
    } else {
  ...
  getCriticalItemCategories(filterOptions.value)
    .then(() => {
  ...
  <div id="customDataTable">
    <div  class="table-responsive max-w-screen" :class="{ 'overflow-x-auto': tableScrollWidth > screenWidth }">
      ...
      <table class="w-full whitespace-no-wrap" >
          ...
          <tfoot v-if="!criticalItemCategories?.data?.length" class="relative h-[250px]">
          <tr v-if="isLoading">
