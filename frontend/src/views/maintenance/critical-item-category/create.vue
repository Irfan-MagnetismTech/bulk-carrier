...
const { criticalItemCategory, storeCriticalItemCategory, isLoading, errors } = useCriticalItemCategory();
const { setTitle } = Title();
const page = 'create';

setTitle('Create Critical Item Category');
</script>
<template>
    <!-- Heading -->
    <div class="flex flex-col items-center justify-between w-full my-3 sm:flex-row" v-once>
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Create New Critical Item Category</h2>
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form @submit.prevent="storeCriticalItemCategory(criticalItemCategory)">
            <!-- Booking Form -->
            <critical-item-category-form :page="page"  v-model:form="criticalItemCategory" :errors="errors"></critical-item-category-form>
            <!-- Submit button -->
            <button type="submit" :disabled="isLoading" class="flex items-center justify-between px-4 py-2 mt-4 text-sm text-white bg-purple-600 border border-transparent rounded-lg fon2t-medium mt- active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">Create Critical Item Category</button>
        </form>
    </div>
</template>