...
<template>
    ...
    <div class="flex flex-col items-center justify-between w-full my-3 sm:flex-row" v-once>
        ...
        <default-button :title="'Job List'" :to="{ name: 'maintenance.job.index' }" :icon="icons.DataBase"></default-button>
    </div>
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form @submit.prevent="storeJob(job)">
            ...
            <!-- Submit button -->
            <button type="submit" :disabled="isLoading" class="flex items-center justify-between px-4 py-2 mt-4 text-sm text-white bg-purple-600 border border-transparent rounded-lg fon2t-medium mt- active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">Create Job</button>
        </form>
    </div>
</template>