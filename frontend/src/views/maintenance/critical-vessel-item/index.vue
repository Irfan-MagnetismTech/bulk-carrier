...
const { criticalVesselItems, getCriticalVesselItems, deleteCriticalVesselItem, isLoading, isTableLoading, errors } = useCriticalVesselItem();
...
function confirmDelete(id) {
  Swal.fire({
    title: 'Are you sure?',
    text: "You want to delete this critical Vessel Item!",
    icon: 'warning',
...
onMounted(() => {
  ...
  getCriticalVesselItems(filterOptions.value)
    .then(() => {
  ...
  <div id="customDataTable">
    ...
    <div  class="table-responsive max-w-screen" :class="{ 'overflow-x-auto': tableScrollWidth > screenWidth }">
      ...
      <table class="w-full whitespace-no-wrap" >
          ...
          <LoaderComponent :isLoading = isTableLoading v-if="isTableLoading && criticalVesselItems?.data?.length"></LoaderComponent>
          </tbody>
          <tfoot v-if="!criticalVesselItems?.data?.length" class="relative h-[250px]">
          <tr v-if="isLoading">
