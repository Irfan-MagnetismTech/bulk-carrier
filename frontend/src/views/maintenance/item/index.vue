...
const { items, getItems, deleteItem, isLoading, isTableLoading, errors } = useItem();
...
watch(
    ...
    (newBusinessUnit, oldBusinessUnit) => {
      if (newBusinessUnit !== oldBusinessUnit) {
        router.push({ name: "mnt.items.index", query: { page: 1 } })
      }
    }
);
let filterOptions = ref( {
  ...
  "filter_options": [
    {
      "rel_type": null,
      "relation_name": "mntItemGroup.mntShipDepartment",
      "field_name": "name",
      "search_param": "",
      "action": null,
      "order_by": null,
    ...
    <div  class="table-responsive max-w-screen" :class="{ 'overflow-x-auto': tableScrollWidth > screenWidth }">
      ...
      <table class="w-full whitespace-no-wrap" >
          ...
          <tr v-for="(item,index) in items?.data" :key="index">
            ...
            <td>
              <nobr>
                <action-button :action="'show'" :to="{ name: 'mnt.items.show', params: { itemId: item?.id } }"></action-button>
