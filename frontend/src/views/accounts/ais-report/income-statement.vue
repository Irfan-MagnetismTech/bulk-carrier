  ...
  <div class="w-full mb-10 income_statement_top_div">
    ...
    <div class="w-full overflow-hidden grid grid-cols-2 gap-1">
      ...
      <table class="w-full">
        <tfoot>
        <tr style="background-color: #E3E3E3">
          <td class="text-sm font-bold text-right" colspan="2">Total Income</td>
          <td class="text-sm font-bold text-right">{{ incomeStatements?.grand_total_income?.toLocaleString('en-IN', {maximumFractionDigits:2}) }}</td>
        </tr>
        </tfoot>
      </table>
    </div>
