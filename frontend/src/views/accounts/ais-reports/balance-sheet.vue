...
// watch(balanceSheets, (newValue, oldValue) => {
//   let liabilitiesTotal = 0;
//   let assetsTotal = 0;
//   newValue.liabilities.forEach((liability) => {
//     let amountWithoutCommas = liability.closing_balance_amount.replace(/,/g, '');
//     liabilitiesTotal += parseFloat(amountWithoutCommas);
//   });
//   newValue.assets.forEach((asset) => {
//     let amountWithoutCommas = asset.closing_balance_amount.replace(/,/g, '');
//     assetsTotal += parseFloat(amountWithoutCommas);
//   });
//   liabilities.value = liabilitiesTotal.toLocaleString('en-IN', {maximumFractionDigits:2})
//   assets.value = assetsTotal.toLocaleString('en-IN', {maximumFractionDigits:2});
// });
  ...
  <form @submit.prevent="getBalanceSheet(searchParams)">
    <div class="w-full flex items-center justify-between mb-2 my-2 select-none">
      <fieldset class="w-full grid grid-cols-4 gap-1 px-2 pb-3 border border-gray-500 rounded dark:border-gray-400">
        <legend class="px-2 text-gray-700 uppercase dark:text-gray-300">Search Balance Sheet</legend>
        <div>
