...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
    ...
    <router-link :to="{ name: 'configuration.vendor.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61] rounded-lg active:bg-[#0F6B61] hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
    ...
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(vendor,index) in vendors" :key="vendor.id">
          ...
          <td class="px-4 py-3 text-sm">{{ vendor.mobile }}</td>
          <td class="px-4 py-3 text-sm">{{ vendor.email }}</td>
          <td class="px-4 py-3 text-sm">{{ vendor.address }}</td>
          <td class="items-center justify-center px-4 py-3 space-x-2 text-sm text-gray-600">
            <action-button :action="'edit'" :to="{ name: 'configuration.vendor.edit', params: { vendorId: vendor.id } }"></action-button>
            <action-button :action="'show'" :to="{ name: 'configuration.vendor.show', params: { vendorId: vendor.id } }"></action-button>
            <action-button @click="deleteVendor(vendor.id)" :action="'delete'"></action-button>
            <!-- <action-button :action="'activity log'" :to="{ name: 'user.activity.log', params: { subject_type: vendor.subject_type,subject_id: vendor.id } }"></action-button> -->
          </td>
        </tr>
        </tbody>
        ...
        <tr v-else-if="!vendors?.length">
          <td colspan="6">No vendor found.</td>
        </tr>
        </tfoot>
      </table>
    </div>
