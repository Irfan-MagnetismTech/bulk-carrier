...
const { branches, getBranches, deleteBranch, isLoading } = useBranch();
...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
    ...
    <router-link :to="{ name: 'configuration.branch.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61]  rounded-lg active:bg-[#0F6B61]  hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
  ...
  <div class="w-full overflow-hidden">
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(branch,index) in branches" :key="branch.id">
          <td class="px-4 py-3 text-sm">{{ index + 1 }}</td>
          <td class="px-4 py-3 text-sm">{{ branch.name }}</td>
          <td class="px-4 py-3 text-sm">{{ branch.branch_code }}</td>
          <td class="px-4 py-3 text-sm">{{ branch.mobile }}</td>
          ...
          <td class="items-center justify-center px-4 py-3 space-x-2 text-sm text-gray-600">
            <action-button :action="'edit'" :to="{ name: 'configuration.branch.edit', params: { branchId: branch.id } }"></action-button>
            <action-button :action="'show'" :to="{ name: 'configuration.branch.show', params: { branchId: branch.id } }"></action-button>
            <action-button @click="deleteBranch(branch.id)" :action="'delete'"></action-button>
            <!-- <action-button :action="'activity log'" :to="{ name: 'user.activity.log', params: { subject_type: branch.subject_type,subject_id: branch.id } }"></action-button> -->
          </td>
        </tr>
        </tbody>
        <tfoot v-if="!branches?.length" class="bg-white dark:bg-gray-800">
        <tr v-if="isLoading">
