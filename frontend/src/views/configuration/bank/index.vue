...
import useBank from "../../../composables/configuration/useBank";
import Title from "../../../services/title";
import {watchEffect, ref, watch} from "vue";
import useDebouncedRef from '../../../composables/useDebouncedRef';
import Paginate from '../../../components/utils/paginate.vue';

const { banks, getBank, deleteBank, isLoading } = useBank();
...
watch(searchKey, newQuery => {
  getBank(props.page, columns, searchKey.value, table);
});
...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-2 sm:flex-row" v-once>
    ...
    <router-link :to="{ name: 'configuration.bank.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61]  rounded-lg active:bg-[#0F6B61]  hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
  ...
  <div class="flex items-center justify-between mb-2 select-none">
    <span class="w-full text-xs font-medium text-gray-500 whitespace-no-wrap">Showing {{ banks?.from }}-{{ banks?.to }} of {{ banks?.total }}</span>
    <!-- Search -->
  ...
  <div class="w-full overflow-hidden">
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(bank,index) in banks?.data" :key="bank.id">
          <td class="px-4 py-3 text-sm">{{  banks?.from + index }}</td>
          <td class="px-4 py-3 text-sm">{{ bank.name }}</td>
          <td class="px-4 py-3 text-sm">{{ bank.code }}</td>
          <td class="items-center justify-center px-4 py-3 space-x-2 text-sm text-gray-600">
            <action-button :action="'edit'" :to="{ name: 'configuration.bank.edit', params: { bankId: bank.id } }"></action-button>
            <action-button :action="'show'" :to="{ name: 'configuration.bank.show', params: { bankId: bank.id } }"></action-button>
        ...
        <tfoot v-if="!banks?.data?.length" class="bg-white dark:bg-gray-800">
        <tr v-if="isLoading">
          <td colspan="5">Loading...</td>
        </tr>
        <tr v-else-if="!banks?.data?.length">
          <td colspan="5">No Bank found.</td>
        </tr>
        </tfoot>
      </table>
    </div>
    <Paginate :data="banks" to="configuration.bank.index" :page="page"></Paginate>
  </div>
</template>
