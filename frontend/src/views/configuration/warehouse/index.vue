...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
    ...
    <router-link :to="{ name: 'configuration.warehouse.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61] rounded-lg active:bg-[#0F6B61] hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
    ...
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr class="text-xs font-semibold tracking-wide text-center text-gray-500 uppercase bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
          ...
          <th class="px-4 py-3">Warehouse Code</th>
          <th class="px-4 py-3">Latest Reading</th>
        ...
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(warehouse,index) in warehouses" :key="warehouse.id">
          <td class="px-4 py-3 text-sm">{{ index + 1 }}</td>
          <td class="px-4 py-3 text-sm">{{ warehouse.name }}</td>
          <td class="px-4 py-3 text-sm">{{ warehouse.code }}</td>
          <td class="px-4 py-3 text-sm">{{ (warehouse.last_reading != null) ? numberFormat(warehouse.last_reading) : numberFormat(warehouse.opening_reading) }}</td>
          <td class="px-4 py-3 text-sm">{{ warehouse.responsible_person }}</td>
          ...
          <td class="items-center justify-center px-4 py-3 space-x-2 text-sm text-gray-600">
            <action-button :action="'edit'" :to="{ name: 'configuration.warehouse.edit', params: { warehouseId: warehouse.id } }"></action-button>
            <action-button :action="'show'" :to="{ name: 'configuration.warehouse.show', params: { warehouseId: warehouse.id } }"></action-button>
            <action-button @click="deleteWarehouse(warehouse.id)" :action="'delete'"></action-button>
            <!-- <action-button :action="'activity log'" :to="{ name: 'user.activity.log', params: { subject_type: warehouse.subject_type,subject_id: warehouse.id } }"></action-button> -->
          </td>
        </tr>
        </tbody>
        <tfoot v-if="!warehouses?.length" class="bg-white dark:bg-gray-800">
        <tr v-if="isLoading">
