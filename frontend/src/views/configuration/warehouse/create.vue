...
const { warehouse, storeWarehouse  } = useWarehouse();
const { setTitle } = Title();

setTitle('Create Warehouse');

onMounted(() => {
    warehouse.value.warehouse_type = 'Warehouse';
    warehouse.value.status = 1;
});
...
<template>
    ...
    <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
        <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Create New Warehouse</h2>
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form @submit.prevent="storeWarehouse(warehouse)">
            <warehouse-form v-model:warehouse="warehouse" :errors="errors"></warehouse-form>
            <!-- Submit button -->
