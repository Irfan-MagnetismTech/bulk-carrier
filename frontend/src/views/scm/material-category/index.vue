...
import useMaterialCategory from "../../../composables/scm/useMaterialCategory";
...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
    <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-200">Material Categories</h2>
    <router-link :to="{ name: 'scm.material-category.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61]  rounded-lg active:bg-[#0F6B61]  hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
...
<!--          <th class="px-4 py-3">Parent Category</th>-->
        ...
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(materialCategory,index) in materialCategories" :key="materialCategory.id">
          <td class="px-4 py-3 text-sm">{{ index + 1 }}</td>
          <td class="px-4 py-3 text-sm">{{ materialCategory.name }}</td>
<!--          <td class="px-4 py-3 text-sm">{{ materialCategory?.parent?.name }}</td>-->
          <td class="px-4 py-3 text-sm">{{ materialCategory.description }}</td>
        ...
        <tfoot v-if="!materialCategories?.length" class="bg-white dark:bg-gray-800">
        <tr v-if="isLoading">
          <td colspan="7">Loading...</td>
        </tr>
        <tr v-else-if="!materialCategories?.length">
