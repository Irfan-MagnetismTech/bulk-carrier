...
import useFrContainerTuesAssign from '../../../composables/commercial/useFrContainerTuesAssign';
import useVoyage from "../../../composables/operation/useVoyage";
import Title from "../../../services/title";
import {watch} from "vue";

const { voyages, voyage:voyageDetail, voyageName, showVoyage, getVoyageName } = useVoyage();
const { voyage, formParams, frContainerParams, isLoading, getFrContainerList, assignFrContainerTues } = useFrContainerTuesAssign();
...
function assignFrContainer(frContainerParams) {

  let data = {
    fr_containers: frContainerParams,
    voyage_id: formParams.value.voyage_id,
  };
  assignFrContainerTues(data);
}
...
watch(voyage,function (val) {
  ...
  frContainerParams.value.length = 0;
  val?.map(function(container, containerIndex) {
    let dataObj = {
      killed_tues: container?.killed_tues ?? '',
      sur_charge_fr_amount: container?.sur_charge_fr_amount ?? '',
    ...
    frContainerParams.value.push(dataObj);
  });
});
...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
    <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Flat Rack (FR) Container List</h2>
  </div>
  <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
    <form class="flex items-end justify-between gap-4" @submit.prevent="getFrContainerList(formParams)">
  ...
  <div>
    <form class="flex items-end justify-between gap-4" @submit.prevent="assignFrContainer(frContainerParams)">
    <div class="w-full overflow-hidden">
        ...
        <table class="w-full mb-8 whitespace-no-wrap">
          <thead v-once>
            <tr class="text-xs font-semibold tracking-wide text-gray-500 uppercase bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
              ...
              <th class="p-1">Killed Tues</th>
              <th class="p-1">Amount</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              <tr class="text-center text-gray-700 dark:text-gray-400" v-for="(container, index) in frContainerParams" :key="index">
                ...
                <td class="p-1 text-center">
                  <input type="number" :class="{'custom_text_opacity' : voyageDetail?.invoices?.length }" v-model="frContainerParams[index].killed_tues" placeholder="Tues" class="w-22 mt-1 text-xs rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" required />
                </td>
                <td class="p-1 text-center">
                  <input type="number" :class="{'custom_text_opacity' : voyageDetail?.invoices?.length }" v-model="frContainerParams[index].sur_charge_fr_amount" step="0.01" placeholder="Amount WWW" class="w-22 mt-1 text-xs rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" required />
                </td>
              </tr>
          </tbody>
      ...
      <button v-if="frContainerParams?.length" type="submit" :class="{'custom_text_opacity' : voyageDetail?.invoices?.length }" :disabled="isLoading" class="w-full px-4 py-2 mb-5 text-sm leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-lg fon2t-medium mt- active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">Submit</button>
    </div>
      </form>
  </div>
</template>
