...
import useManualDevitNote from "../../../composables/commercial/useManualDevitNote";

const route = useRoute();
const store = useStore();
const { setTitle } = Title();
const dropZoneFile = ref(computed(() => store.getters.getDropZoneFile));
const { devitNotes, process, isLoading, errors } = useManualDevitNote();
const isTooltipShowing = ref(false);

setTitle('Manual Debit Note');

watch(dropZoneFile, (value) => {
    if (value !== null && value !== undefined) {
      devitNotes.file = value;
    }
});
...
<template>
    <Heading :label="'Manual Debit Note'" type="none"></Heading>
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <DropZone></DropZone>
        <form v-if="dropZoneFile !== null" @submit.prevent="process(devitNotes)" class="flex flex-col items-center justify-center w-full gap-2 mt-2" enctype="multipart/form-data">
