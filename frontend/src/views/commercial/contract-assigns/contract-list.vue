...
const message = 'Invoice already generated for this voyage.';
...
function loadContractAssignData(customerId,rateType,sector,voyageCustomerId,hasInvoice) {
  if(hasInvoice){
...
<template>
  ...
  <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              ...
              <template v-for="(voyageCustomer, voyageCustomerKey, voyageCustomerIndex) in openContractCustomerList" :key="voyageCustomerIndex">
                <tr class="text-gray-700 dark:text-gray-400" v-for="(sector,sectorKey, index) in voyageCustomer" :key="sectorKey">
                  <td :rowspan="Object.keys(voyageCustomer).length" v-if="sectorKey == 0">
                    <button type="button" :class="{'custom_opacity' : sector?.has_invoice }" :disabled="isLoading" @click="moveToFixedContract(voyageCustomerKey)" class="w-full px-2 py-2 text-sm text-white bg-purple-600 border border-transparent rounded-lg hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"><nobr>Move to DF</nobr></button>
                  </td>
                  <td :rowspan="Object.keys(voyageCustomer).length" v-if="sectorKey == 0">
                  ...
                  <td><button type="button" :class="{'custom_text_opacity' : sector?.has_invoice }" title="Contract List" @click="loadContractAssignData(sector?.customer_id,'Open',sector?.sector,sector?.voyage_customer_id,sector?.has_invoice)" :disabled="isLoading" class="w-full px-2 py-2 text-sm text-white bg-purple-600 border border-transparent rounded-lg hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"><nobr>Contract List</nobr></button></td>
                </tr>
              </template>
              </tbody>
          ...
          <div class="w-full md:overflow-x-auto">
            <table class="w-full mt-2 whitespace-no-wrap border dark:border-0 table2">
              ...
              <template v-for="(voyageCustomer, voyageCustomerKey, voyageCustomerIndex) in fixedContractCustomerList" :key="voyageCustomerIndex">
                <tr class="text-gray-700 dark:text-gray-400" v-for="(sector,sectorKey, index) in voyageCustomer" :key="sectorKey">
                  <td :rowspan="Object.keys(voyageCustomer).length" v-if="sectorKey == 0">
                    <button type="button" :class="{'custom_opacity' : sector?.has_invoice }" @click="moveToOpenContract(voyageCustomerKey)" :disabled="isLoading" class="w-full px-2 py-2 text-sm text-white bg-purple-600 border border-transparent rounded-lg hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"><nobr>Move to OS</nobr></button>
                  </td>
                  <td :rowspan="Object.keys(voyageCustomer).length" v-if="sectorKey == 0">
                    <strong>{{ sector?.customer_code }}</strong> <br>
                    {{ sector?.company_name }}
                  ...
                  <td :rowspan="Object.keys(voyageCustomer).length" v-if="sectorKey == 0">
                    <button type="button" title="Contract List" :class="{'custom_text_opacity' : sector?.has_invoice }" @click="loadContractAssignData(sector?.customer_id,'Fixed',sector?.sector,sector?.voyage_customer_id,sector?.has_invoice)" :disabled="isLoading" class="w-full px-2 py-2 text-sm text-white bg-purple-600 border border-transparent rounded-lg hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"><nobr>Contract List</nobr></button>
                  </td>
                </tr>
              </template>
              </tbody>
          ...
          <table class="w-full mt-2 whitespace-no-wrap border dark:border-0 table2">
            ...
            <template v-for="(voyageCustomer, voyageCustomerKey, voyageCustomerIndex) in forceContractCustomerList" :key="voyageCustomerIndex">
              <tr class="text-gray-700 dark:text-gray-400" v-for="(sector,sectorKey, index) in voyageCustomer" :key="sectorKey">
                ...
                <td><button type="button" title="Contract List" :class="{'custom_text_opacity' : sector?.has_invoice }" @click="loadContractAssignData(sector?.customer_id,'Force',sector?.sector,sector?.voyage_customer_id,sector?.has_invoice)" :disabled="isLoading" class="w-full px-2 py-2 text-sm text-white bg-purple-600 border border-transparent rounded-lg hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"><nobr>Contract List</nobr></button></td>
              </tr>
            </template>
            </tbody>
    ...
    <div v-show="isContractAssignModalOpen" class="fixed inset-0 z-30 bg-black bg-opacity-50 sm:items-center sm:justify-center overflow-y-scroll">
      <!-- Modal -->
      <div style="left: 50%;position: relative;transform: translateX(-50%);" class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
