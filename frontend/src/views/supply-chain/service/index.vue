...
const { services, getServices, deleteService, isLoading, } = useMaterialCategory();
...
<template>
  ...
  <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
    ...
    <router-link :to="{ name: 'supply-chain.service.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61]  rounded-lg active:bg-[#0F6B61]  hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
    ...
    <div class="w-full overflow-x-auto">
      <table class="w-full whitespace-no-wrap">
        ...
        <tr class="text-gray-700 dark:text-gray-400 text-center" v-for="(service,index) in services" :key="service.id">
          ...
          <td class="px-4 py-3 text-sm">{{ service.short_code }}</td>
