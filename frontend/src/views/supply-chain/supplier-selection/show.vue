...
<template>
  ...
  <label class="label-group">
        ...
        <span class="label-item-title"> {{ quotation.ref_no }}</span>
    </label>
    <label class="label-group">
        <span class="label-item-title font-bold">Warehouse<span class="text-red-500"> : </span></span>
        <span class="label-item-title"> {{ quotation.scmWarehouse?.name }}</span>
    </label>
    <label class="label-group">
        <span class="label-item-title font-bold">Expire Date<span class="text-red-500"> : </span></span>
        <span class="label-item-title"> {{ formatDate(quotation.expire_date) }}</span>
    </label>
  </div>
  ...
  <div class="input-group">
    <label class="label-group">
        ...
        <span class="label-item-title"> {{ quotation.priority }}</span>
    </label>
    <label class="label-group">
        <span class="label-item-title font-bold">Required Days<span class="text-red-500"> : </span></span>
        <span class="label-item-title"> {{ quotation.required_days }}</span>
    </label>
    
    <label class="label-group">
        <span class="label-item-title font-bold">Effective Date<span class="text-red-500"> : </span></span>
        <span class="label-item-title"> {{ formatDate(quotation.effective_date) }}</span>
    </label>
  </div>
...
<table class="min-w-full divide-y divide-gray-200 dark-disabled:divide-gray-700 mt-10">
  ...
  <thead class="bg-gray-50 dark-disabled:bg-gray-800">
    <tr class="text-gray-600 dark-disabled:text-gray-400 text-sm leading-normal">
      ...
      <th class="px-6 py-3 text-center text-gray-600 dark-disabled:text-gray-400 uppercase tracking-wider" v-for="(vendorData,index) in (CsData?.scmCsVendor)" :key="index">{{ vendorData[0].scmVendor?.name }}</th>       
    </tr> 
  </thead>
  <tbody>
    <tr>
      <td>Quotation Ref</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].quotation_ref }}</td>
    </tr>
    <tr>
      <td>Quotation Received Date</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ formatDate(VendoData[0].quotation_received_date) }}</td>
    </tr>
    <tr>
      <td>Offer Validity</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].quotation_validity }}</td>
    </tr>
    <tr>
      <td>Sourcing History</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].sourcing }}</td>
    </tr>
    <tr>
      <td>Date Of RFQ</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ formatDate(VendoData[0].date_of_rfq) }}</td>
    </tr>
    <tr>
      <td>Party Type</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].scmVendor.product_source_type }}</td>
    </tr>
    <tr>
      <td>Delivery Time</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].delivery_time }}</td>
    </tr>
    <tr v-if="quotation.purchase_center == 'Foreign'">
      <td>Estimated Shipment Date</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ formatDate(VendoData[0].quotation_shipment_date) }}</td>
    </tr>
    <tr v-if="quotation.purchase_center == 'Foreign'">
      <td>Port Of Loading</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].port_of_loading }}</td>
    </tr>
    <tr v-if="quotation.purchase_center == 'Foreign'">
      <td>Port Of Discharge</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].port_of_discharge }}</td>
    </tr>
    <tr v-if="quotation.purchase_center == 'Foreign'">
      <td>Mode Of Shipment</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].mode_of_shipment }}</td>
    </tr>
    <tr>
      <td>Currency</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].currency }}</td>
    </tr>
    <tr>
      <td>Remarks</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].remarks }}</td>
    </tr>
    <tr>
      <td>Terms and Condition</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].terms_and_condition }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>VAT</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].vat }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>AIT</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].ait }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Payment Method</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].payment_method }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Inventory Type</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].stock_type }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Manufacturing Days</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].manufacturing_days }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Credit Term</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].credit_term }}</td>
    </tr>
    <tr>
      <td>Inco-term</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].delivery_term }}</td>
    </tr> 
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Carring Charge Bear By</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].carring_cost_bear_by }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Unloading Charge Bear By</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].unloading_cost_bear_by }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Unloading Charge Bear By</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].unloading_cost_bear_by }}</td>
    ...
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Offer Price</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].total_offered_price }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Freight</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].freight }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Total Offer Price</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].grand_total_offered_price }}</td>
    </tr>
    <tr v-if="quotation.purchase_center != 'Foreign'">
      <td>Negotiated Price</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">{{ VendoData[0].total_negotiated_price }}</td>
    </tr>
    <tr>
      <td>Technical Acceptance </td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">
            <!-- <select v-model="quotation.scmCsVendor[index][0].technical_acceptance" class="quotation-input">
              <option value="Accepted">Accepted</option>
              <option value="Rejected">Rejected</option>
            </select> -->
            {{ quotation.scmCsVendor[index][0].technical_acceptance }}
      </td>
    </tr>
    <tr>
      <td>Selected Vendor</td>
      <td v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">
        <!-- <input type="checkbox" :class="'menu_' + VendoData.scm_cs_id" class="text-purple-600 quotation-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray" :value="VendoData?.id" v-model="quotation.scmCsVendor[index][0].is_selected"> -->
        {{ quotation.scmCsVendor[index][0].is_selected ? 'Selected' : 'Not Selected' }}
      </td>
    </tr>
  </tbody>
  </table>  
...
<table class="min-w-full divide-y divide-gray-200 dark-disabled:divide-gray-700 mt-10">
  ...
  <div v-show="isModalOpen" class="fixed inset-0 z-30 flex items-end overflow-y-auto bg-black bg-opacity-50 sm:items-center sm:justify-center">
    <!-- Modal -->
    <quotation @submit.prevent="" style="position: absolute;top: 0;">
      <div class="w-full px-6 py-4 overflow-y-auto bg-white rounded-t-lg dark-disabled:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
        ...
        <table class="w-full mb-2 whitespace-no-wrap border-collapse contract-assign-table table2">
          ...
          <tr style="background-color: #04AA6D;color: white"
            ...
            <!-- <th colspan="5">{{ details }}</th>
            <template v-for="(VendoData,name,index) in (CsData?.scmCsVendor)" :key="index" v-if="index == 0">
        ...
        <div class="dt-responsive table-responsive">
          <table id="dataTable" class="w-full table table-striped table-bordered">
            <thead>
              <tr>
                <th>Description</th>
                <th scope="col" class="px-6 py-3 text-left text-gray-600 dark-disabled:text-gray-400 uppercase tracking-wider" v-for="(vendorData,index) in (CsData?.scmCsVendor)" :key="index">{{ vendorData[0].scmVendor?.name }}</th> 
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>Brand</td>
                <template v-for="(VendoData,index) in (CsData?.scmCsVendor)" :key="index">
