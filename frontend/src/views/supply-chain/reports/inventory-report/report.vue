...
<template>
    ...
    <div class="flex flex-col items-center justify-between w-full my-3 sm:flex-row" v-once>
        <h2 class="text-2xl font-semibold text-gray-700 dark-disabled:text-gray-200">Inventory Report</h2>
    </div>
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark-disabled:bg-gray-800">
        <form @submit.prevent="inventoryReport(formParams)">
            <!-- Form -->

            <inventory-report-form :page="page" v-model:form="formParams" :errors="errors"></inventory-report-form>
    ...
    <div v-show="showReport">
        <div v-if="inventory?.length" class="">
                        ...
                        <table id="table" class="w-full whitespace-no-wrap">
                            ...
                            <tbody class="bg-white divide-y dark-disabled:divide-gray-700 dark-disabled:bg-gray-800">
                                <tr v-for="(inv, index) in inventory" :key="index">
                                    <td>{{ index+1 }}</td>
                                    <td class="!text-left">{{ inv?.scmMaterial?.material_name_and_code }}</td>
                                    <td>{{ inv?.scmMaterial?.unit }}</td>
                                    <td class="!text-right">{{ inv?.opening_stock }}</td>
                                    <td class="!text-right">{{ inv?.received }}</td>
                                    <td class="!text-right">{{ inv?.consumed }}</td>
                                    <td class="!text-right">{{ inv?.closing_stock }}</td>
