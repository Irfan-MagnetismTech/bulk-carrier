...
const { purchaseRequisition, showPurchaseRequisition  } = usePurchaseRequisition();
...
<template>
    <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
        <h2 class="text-xl font-semibold text-gray-700 dark-disabled:text-gray-200">Purchase Requisition</h2>
        <router-link :to="{ name: 'scm.purchase-requisitions.index' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-[#0F6B61]  border border-transparent rounded-lg active:bg-[#0F6B61]  hover:bg-[#0F6B90] focus:outline-none focus:shadow-outline-purple">
    ...
    <div class="w-full overflow-x-auto">
      <table class="w-2/6 whitespace-no-wrap mb-4">
        ...
        <tr>
          ...
          <td class="px-2">{{ purchaseRequisition.date ? moment(purchaseRequisition.date).format('DD-MM-YYYY') : null }}</td>
        </tr>
        <tr>
          <td class="px-2">Note</td>
          <td class="px-2">{{ purchaseRequisition?.note }}</td>
        </tr>
        <tr>
          <td class="px-2">Remarks</td>
          <td class="px-2">{{ purchaseRequisition?.remarks }}</td>
        </tr>
        <tr>
      ...
      <table class="w-full whitespace-no-wrap">
        <thead>
          <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark-disabled:border-gray-700 bg-gray-50 dark-disabled:text-gray-400 dark-disabled:bg-gray-800">
        ...
        <tbody class="bg-white dark-disabled:divide-gray-700 dark-disabled:bg-gray-800">
          <tr v-for="(stockData,index) in purchaseRequisition?.scmPrLines" :key="index">
            <td class="px-2">{{ stockData?.scmMaterial?.name }}</td>
...
<style lang="postcss" scoped>
    ...
    span.label-item-input {
        @apply bg-gray-100 py-2 h-9 flex items-center border border-black px-3 w-full mt-1 text-xs rounded dark-disabled:text-gray-300 dark-disabled:border-gray-600 dark-disabled:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray
    }
