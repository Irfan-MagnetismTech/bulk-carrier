<template>
  <div class="flex items-center justify-between w-full my-3" v-once>
    <h2 class="text-2xl font-semibold text-gray-700 dark-disabled:text-gray-200">Store Issue Details</h2>
            ...
            <table class="w-full">
                ...
                <tbody>
                    <tr>
                        ...
                        <td><span :class="storeIssue?.business_unit === 'PSML' ? 'text-green-700 bg-green-100' : 'text-orange-700 bg-orange-100'" class="px-2 py-1 font-semibold leading-tight rounded-full">{{ storeIssue?.business_unit }}</span></td>
                    </tr>
                    <tr>
                        <th class="w-40">Store Issue No.</th>
                    ...
                    <tr>
                        ...
                        <td>{{ storeIssue?.scmSr?.ref_no }}</td>
                    </tr>
                    <tr>
                        <th class="w-40">Remarks </th>
                        <td>{{ storeIssue?.remarks }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
    <div class="flex md:gap-4 mt-1 md:mt-2" v-if="storeIssue.scmSiLines?.length">
        <div class="w-full">
          <table class="w-full">
            <thead>
                <tr>
            ...
            <tbody>
              <tr v-for="(certificate, index) in storeIssue.scmSiLines">
                <td>
                  {{ index+1 }}
                </td>
                <td>
                  <span v-if="storeIssue.scmSiLines[index]?.scmMaterial?.name">{{ storeIssue.scmSiLines[index]?.scmMaterial?.name }}</span>
                </td>
                <td>
                  <span v-if="storeIssue.scmSiLines[index]?.unit">{{ storeIssue.scmSiLines[index]?.unit }}</span>
                </td>
                <td>
                <span>
                    {{ numberFormat(storeIssue.scmSiLines[index].quantity) }}
                </span>
              </td>
              </tr>
            </tbody>
