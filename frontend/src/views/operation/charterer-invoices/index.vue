...
const { chartererInvoices, getChartererInvoices, deleteChartererInvoice, isLoading } = useChartererInvoice();
...
onMounted(() => {
  watchEffect(() => {
    getChartererInvoices(props.page, businessUnit.value)
    .then(() => {
...
<template>
      ...
      <table class="w-full whitespace-no-wrap" >
          ...
          <tbody v-if="chartererInvoices?.data?.length">
              <tr v-for="(chartererInvoice, index) in chartererInvoices.data" :key="chartererInvoice?.id">
                  ...
                  <td>
                    {{ 
                  chartererInvoice?.opsVoyage?.opsVoyageSectors.reduce((accumulator, currentObject) => {
  ...
  return accumulator + currentObject.final_received_qty
          ...
          <tfoot v-if="!chartererInvoices?.length">
          <tr v-if="isLoading">
