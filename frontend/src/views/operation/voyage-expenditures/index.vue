...
setTitle('Voyage Expenditure List');
...
onMounted(() => {
  watchPostEffect(() => {
    ...
    getVoyageExpenditures(filterOptions.value)
      .then(() => {
      ...
      <table class="w-full whitespace-no-wrap mb-5" >
        <FilterComponent :filterOptions = "filterOptions"/>
          <tbody v-if="voyageExpenditures?.data?.length" class="relative">
              ...
              <LoaderComponent :isLoading = isTableLoading v-if="isTableLoading && voyageExpenditures?.data?.length"></LoaderComponent>
          </tbody>
          <tfoot v-if="!voyageExpenditures?.data?.length" class="relative h-[250px]">
            <tr v-if="isLoading">
            </tr>
            ...
            <tr v-else-if="!voyageExpenditures?.data?.length">
