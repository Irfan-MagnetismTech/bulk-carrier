...
<template>
    ...
    <div class="flex flex-col items-center justify-between w-full my-6 sm:flex-row" v-once>
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Port List</h2>
        <router-link :to="{ name: 'operation.configurations.ports.create' }" class="flex items-center justify-between gap-1 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                ...
                <nav aria-label="Table navigation">
                    <ul class="inline-flex items-center">
                        <li v-for="(link, index) in ports?.links" :key="index">
                            <router-link :disabled="ports.current_page === 1" :to="{ name: 'operation.configurations.ports.index', query: { page: ports.current_page >= 2 ? ports.current_page - 1 : 1 } }" v-if="link.label.includes('Previous')" class="paginate-btn">
                            ...
                            <router-link v-else-if="link.label.includes('Next')" :disabled="ports.current_page === ports.last_page" :to="{ name: 'operation.configurations.ports.index', query: { page: ports.current_page <= ports.last_page - 1 ? ports.current_page + 1 : ports.last_page } }" class="paginate-btn">
                            ...
                            <router-link v-else :to="{ name: 'operation.configurations.ports.index', query: { page: parseInt(link.label) } }" class="paginate-btn" :class="{ 'paginate-active': link?.active }">
                                <span>{{ link?.label }}</span>
                            </router-link>
                        </li>
                    </ul>
