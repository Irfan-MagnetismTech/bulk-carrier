...
import CopinoTable from '../../../components/operation/CopinoTable.vue';
import useCopino from '../../../composables/operation/useCopino.js';
...
const { copino, errors, allCopinos, process, isLoading, error } = useCopino();
const { voyage, showVoyage } = useVoyage();

copino.voyage_id = route.params.voyageId;

watch(voyage, (value) => {
    if (value === null || value === undefined) {
        setTitle('Copino');
    } else {
        setTitle(`${value.voyage} - Copino`);
    }
});
...
<template>
    <Heading :label="`COPINO: #${voyage?.voyage}`" type="none"></Heading>
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <DropZone></DropZone>
        <form v-if="dropZoneFile !== null" @submit.prevent="process(copino)" class="flex flex-col items-center justify-center w-full gap-2 mt-2" enctype="multipart/form-data">
            <input v-model.number="copino.start" type="number" required min="0" placeholder="Line Starts From" class="label-item-input" />
    ...
    <CopinoTable :data="allCopinos"></CopinoTable>
</template>
