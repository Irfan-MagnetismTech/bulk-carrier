<template>
            ...
            <table class="w-full">
              ...
              <tbody>
                  <tr>
                      ...
                      <td><span :class="vesselExpenseHead?.business_unit === 'PSML' ? 'text-green-700 bg-green-100' : 'text-orange-700 bg-orange-100'" class="px-2 py-1 font-semibold leading-tight rounded-full">{{ vesselExpenseHead?.business_unit }}</span></td>
                  </tr>
                  <tr>
  ...
  <div class="relative" v-if="vesselExpenseHead?.heads?.length > 0">
        ...
        <fieldset class="px-4 pb-4 mt-3 border border-gray-700 rounded dark-disabled:border-gray-400">
          <legend class="px-2 rounded !text-center font-bold bg-green-600 uppercase text-white">Cost Group and Cost Heads </legend>
          <div class="mt-2">
            <div :id="'cost_' + index" :class="index%2===0 ? 'bg-gray-100' : 'bg-yellow-100'" style="position: relative" class="px-2 py-2 border sm:rounded-lg mb-1" v-for="(costGroup, index) in vesselExpenseHead.heads" :key="index">
              ...
              <label class="flex items-center mb-2 text-gray-600 text-xs">
              ...
              <template v-for="(subhead,subIndex) in vesselExpenseHead.heads[index]?.opsSubHeads" :key="subIndex">
                <label class="flex ml-6 items-center mb-2 text-gray-600 text-xs">
