...
import useCrewDocument from '../../../composables/crew/useCrewDocument.js';
...
const { crewDocument, showCrewDocument, errors } = useCrewDocument();

const { setTitle } = Title();

setTitle('Crew Document Details');
...
<template>
  <div class="flex items-center justify-between w-full my-3" v-once>
    <h2 class="text-2xl font-semibold text-gray-700 dark-disabled:text-gray-200"> Crew Document Details  # {{documentId}} </h2>
        ...
        <table class="w-full">
          ...
          <tbody>
            <tr>
              ...
              <td><span :class="crewDocument?.business_unit === 'PSML' ? 'text-green-700 bg-green-100' : 'text-orange-700 bg-orange-100'" class="px-2 py-1 font-semibold leading-tight rounded-full">{{ crewDocument?.business_unit }}</span></td>
            </tr>
            <tr>
              <th class="w-40"> Vessel Name </th>
              <td>{{ crewDocument?.opsVessel?.name }}</td>
            </tr>
            <tr>
              <th class="w-40"> Total Crews </th>
              <td>{{ crewDocument?.total_crews }}</td>
            </tr>
            <tr>
              <th class="w-40"> Year - Month </th>
              <td>{{ crewDocument?.year_month }}</td>
            </tr>
            <tr>
              <th class="w-40"> Working Days </th>
              <td>{{ crewDocument?.working_days }}</td>
            </tr>
            <tr>
              <th class="w-40"> Remarks </th>
              <td>{{ crewDocument?.remarks }}</td>
            </tr> 
          </tbody>
        </table>
