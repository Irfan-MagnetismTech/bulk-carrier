...
onMounted(() => {
  watchPostEffect(() => {
    if(currentPage.value == props.page && currentPage.value != 1) {
      ...
      router.push({ name: 'crw.crwAttendances.index', query: { page: filterOptions.value.page } });
    } else {
    ...
    getCrwAttendances(filterOptions.value)
        .then(() => {
...
<!--                <action-button :action="'show'" :to="{ name: 'crw.crwAttendances.show', params: { crwAttendanceId: crwAttendance?.id } }"></action-button>-->
          ...
          <LoaderComponent :isLoading = isTableLoading v-if="isTableLoading && crwAttendances?.data?.length"></LoaderComponent>
          </tbody>
        <tfoot v-if="!crwAttendances?.data?.length" class="relative h-[250px]">
        <tr v-if="isLoading">
          <td colspan="7"></td>
        </tr>
        ...
        <tr v-else-if="!crwAttendances?.data?.data?.length">
