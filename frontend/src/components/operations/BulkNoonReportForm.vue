...
<!-- General -->
          ...
          <table id="dataTable" class="w-full table table-striped table-bordered">
            ...
            <tbody>
              <tr v-for="(bunker, index) in form.opsBulkNoonReportConsumptions" :key="index">
                  ...
                  <td>
                    <input type="text" class="form-input" v-model.trim="form.opsBulkNoonReportConsumptions[index].previous_rob" @input="calculateRob(index)">
                  </td>
                  <td>
                    <input type="text" class="form-input" v-model.trim="form.opsBulkNoonReportConsumptions[index].received" @input="calculateRob(index)">
                  </td>
                  <td>
                    <input type="text" class="form-input bg-gray-100" readonly v-model.trim="form.opsBulkNoonReportConsumptions[index].consumption">
                  </td>
                  <td>
                    <input type="text" class="form-input bg-gray-100" readonly v-model.trim="form.opsBulkNoonReportConsumptions[index].rob">
                  </td>
                  <td>
  ...
  <div v-show="isBunkerConsumptionModalOpen" class="fixed inset-0 z-30 flex items-end overflow-y-auto bg-black bg-opacity-50 sm:items-center sm:justify-center">
    <!-- Modal -->
    <form @submit.prevent="pushBunkerConsumption()" style="position: absolute;top: 0;">
      <div class="w-full px-6 py-4 overflow-y-auto bg-white rounded-t-lg dark-disabled:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
        ...
        <div class="dt-responsive table-responsive">
          <table id="dataTable" class="w-full table table-striped table-bordered">
            <thead>
              <tr>
                <th>Head <span class="text-red-500">*</span></th>
                <th>Quantity <span class="text-red-500">*</span></th>
                <th>
                  <button type="button" @click="addConsumptionHead()" class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                      <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                    </svg>
                  </button>
            ...
            <tbody>
              <tr v-for="(details, index) in bunkerConsumptionDetails" :key="index">
                ...
                <td>
                  <input type="number" step="0.001" v-model.trim="details.amount" required class="form-input text-right" placeholder="Quantity" />
                </td>
                <td>
                  <button type="button" @click="removeConsumptionHead(index)" class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                    </svg>
                  </button> 
...
function pushBunkerConsumption() {
  ...
  let total = bunkerConsumptionDetails.value.reduce((accumulator, currentObject) => {
    const amount = currentObject.amount ?? 0;
  ...
  props.form.opsBulkNoonReportConsumptions[currentConsumptionIndex.value].consumption = total

  calculateRob(currentConsumptionIndex.value)

}

function calculateRob(index) {

  let previous_rob = props.form.opsBulkNoonReportConsumptions[index].previous_rob ?? 0;

  let received = props.form.opsBulkNoonReportConsumptions[index].received ?? 0;

  let consumption = props.form.opsBulkNoonReportConsumptions[index].consumption ?? 0;

  props.form.opsBulkNoonReportConsumptions[index].rob =  parseFloat(previous_rob * 1 + received * 1 - consumption * 1).toFixed(2)
}
