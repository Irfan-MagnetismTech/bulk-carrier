<template>
  <div class="justify-center w-full grid grid-cols-1 md:grid-cols-3 md:gap-2">
      ...
      <label class="block w-full mt-2 text-sm">
          ...
          <v-select placeholder="--Choose an option--" :loading="isCriticalFunctionLoading"  :options="criticalFunctions" @search="" v-model="form.mnt_critical_function_name" label="function_name" class="block w-full mt-1 text-sm rounded dark-disabled:text-gray-300 dark-disabled:border-gray-600 dark-disabled:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray form-input">
        ...
        <label class="block w-full mt-2 text-sm">
            ...
            <v-select placeholder="--Choose an option--" :loading="isCriticalItemCategoryLoading"  :options="form.mntItemCategories" @search="" v-model="form.mnt_critical_item_cat_name" label="category_name" class="block w-full mt-1 text-sm rounded dark-disabled:text-gray-300 dark-disabled:border-gray-600 dark-disabled:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray form-input">
            <template #search="{attributes, events}">
            <input
                class="vs__search"
...
watch(() => props.form.mnt_critical_item_cat_name, (value) => {
  props.form.mnt_critical_item_cat_id = value?.id; 
});
