        ...
        <li class="relative ml-2 rounded-md"  style="width: calc(100% - 14px);">
          <div class="inline-flex items-center justify-between w-full text-sm font-semibold transition-colors rounded-md group group-hover:text-white duration-200 ease-linear p-2" aria-haspopup="true">
                      <span class="inline-flex items-center">
                        <router-link :to="{ name: priceSubMenu.route }" class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-white">
                          <span class="ml-2 font-light ">{{ priceSubMenu.label }}</span>
                        </router-link>
                      </span>
         
            <div v-if="priceSubMenu.subSubMenu.length" @click="revealGrandChilds(element.subMenu.length+priceSubMenu.subSubMenu.length)" v-html="element.icon" class="duration-200 ease-linear" :class="{ 'rotate-180': grandChildsVisible }"></div>

          </div>
          <ul v-if="priceSubMenu.subSubMenu.length" :class="{ '': grandChildsVisible }" :style="{ height: grandChildsVisible ? (42 * priceSubMenu.subSubMenu.length)+'px' : '0' }" class="collapse overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-[#363657] dark:text-gray-400 dark:bg-gray-900 grand-child" aria-label="submenu">
            <li v-for="priceSubSubMenu in priceSubMenu.subSubMenu" class="p-2 transition-colors duration-150  dark:hover:text-gray-200 rounded-md hover:bg-indigo-900 duration-200 ease-linear p-2" :class="{ 'active': isActive === 'New Fixed Contract' }">
