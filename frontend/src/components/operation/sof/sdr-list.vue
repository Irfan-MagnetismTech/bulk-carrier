...
<template>
  <table class="w-full whitespace-no-wrap mt-2 border-collapse">
    ...
    <tr class="text-gray-700 dark:text-gray-400" v-for="(listData,index) in voyageSofLists" :key="index">
      ...
      <td>{{ listData?.incident_port }}</td>
      <td>{{ listData?.sdr_no }}</td>
      <td>{{ listData?.responsible_party }}</td>
      <td>{{ listData?.estimated_cost }}</td>
      <td>{{ listData?.sdr_status }}</td>
      ...
      <td>
        <div class="flex items-center justify-center space-x-2 text-gray-600">
          ...
          <action-button :action="'delete'" @click="deleteVoyageSofList(listData?.id,voyageId,'sdr')"></action-button>
        </div>
      </td>
    </tr>
    </tbody>
    ...
    <tr v-else-if="!voyageSofLists?.length">
      <td colspan="10" class="text-center">No sdr list found.</td>
    </tr>
    </tfoot>
  </table>
  <div v-show="isVoyageSofModalOpen" class="fixed inset-0 z-30 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
    <!-- Modal -->
    <form @submit.prevent="storeVoyageSofList(voyageSof,voyageId,'sdr')">
    <div class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
      ...
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        ...
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Incident Date <span class="text-red-500">*</span></span>
        ...
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Incident Port <span class="text-red-500">*</span></span>
          <select v-model="voyageSof.incident_port" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" required>
            <option value="">Select Port</option>
            <option v-for="port in uniqueServicePorts" :value="port">{{ port }}</option>
          </select>
          <Error v-if="errors?.incident_port" :errors="errors.incident_port" />
        </label>
      </div>
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">SDR No.<span class="text-red-500">*</span></span>
          <input type="text" v-model="voyageSof.sdr_no" required placeholder="Sdr no" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.sdr_no" :errors="errors.sdr_no" />
        </label>
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Responsible Party <span class="text-red-500">*</span></span>
          <input type="text" v-model="voyageSof.responsible_party" required placeholder="Responsible party"  class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.responsible_party" :errors="errors.responsible_party" />
        </label>
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Estimated Cost<span class="text-red-500">*</span></span>
          <input type="text" v-model="voyageSof.estimated_cost" required placeholder="Estimated Cost" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.estimated_cost" :errors="errors.estimated_cost" />
        </label>
      </div>
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">SDR Status<span class="text-red-500">*</span></span>
          <input type="text" v-model="voyageSof.sdr_status" required placeholder="SDR status" class="block w-2/6 mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.sdr_status" :errors="errors.sdr_status" />
        </label>
      </div>
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm">
