...
<template>
  <table class="w-full whitespace-no-wrap mt-2 border-collapse">
    <thead>
    <tr>
      <th colspan="8" class="text-center font-bold bg-red-500 text-white">
        Reefer List
      </th>
    ...
    <tr class="text-gray-700 dark:text-gray-400" v-for="(listData,index) in voyageSofLists" :key="index">
      ...
      <td>
        <div class="flex items-center justify-center space-x-2 text-gray-600">
          ...
          <action-button :action="'delete'" @click="deleteVoyageSofList(listData?.id,voyageId,'reefer')"></action-button>
        </div>
      </td>
    </tr>
    </tbody>
    ...
    <tr v-else-if="!voyageSofLists?.length">
      <td colspan="9" class="text-center">No reefer list found.</td>
    </tr>
    </tfoot>
  </table>
  <div v-show="isVoyageSofModalOpen" class="fixed inset-0 z-30 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
    <!-- Modal -->
    <form @submit.prevent="storeVoyageSofList(voyageSof,voyageId,'reefer')">
    <div class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
      ...
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Temp<span class="text-red-500">*</span></span>
          <input type="number" step=".01" v-model="voyageSof.temp" required placeholder="Temp" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.temp" :errors="errors.temp" />
        </label>
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Loading Temp <span class="text-red-500">*</span></span>
          <input type="number" step=".01" v-model="voyageSof.loading_temp" required placeholder="Loading Temp"  class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.loading_temp" :errors="errors.loading_temp" />
        </label>
        <label class="block w-full mt-3 text-sm">
