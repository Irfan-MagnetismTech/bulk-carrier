...
<template>
  <table class="w-full whitespace-no-wrap mt-2 border-collapse">
    <thead>
    <tr>
      <th colspan="10" class="text-center font-bold bg-red-500 text-white">
        CDR List
      </th>
    ...
    <tr class="text-gray-700 dark:text-gray-400" v-for="(listData,index) in voyageSofLists" :key="index">
      ...
      <td>{{ listData?.pol }}</td>
      <td>{{ listData?.pod }}</td>
      <td>{{ listData?.damage_nature }}</td>
      <td>{{ listData?.master_acknowledgement }}</td>
      ...
      <td>
        <div class="flex items-center justify-center space-x-2 text-gray-600">
          ...
          <action-button :action="'delete'" @click="deleteVoyageSofList(listData?.id,voyageId,'cdr')"></action-button>
        </div>
      </td>
    </tr>
    </tbody>
    ...
    <tr v-else-if="!voyageSofLists?.length">
      <td colspan="11" class="text-center">No cdr list found.</td>
    </tr>
    </tfoot>
  </table>
  <div v-show="isVoyageSofModalOpen" class="fixed inset-0 z-30 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
    <!-- Modal -->
    <form @submit.prevent="storeVoyageSofList(voyageSof,voyageId,'cdr')">
    <div class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
      ...
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Container<span class="text-red-500">*</span></span>
      ...
      <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Damage Nature<span class="text-red-500">*</span></span>
          <input type="text" v-model="voyageSof.damage_nature" required placeholder="Damage nature" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.damage_nature" :errors="errors.damage_nature" />
        </label>
        <label class="block w-full mt-3 text-sm">
          <span class="text-gray-700 dark:text-gray-300">Damage Nature<span class="text-red-500">*</span></span>
          <input type="text" v-model="voyageSof.master_acknowledgement" required placeholder="Master acknowledgement" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
          <Error v-if="errors?.master_acknowledgement" :errors="errors.master_acknowledgement" />
        </label>
      </div>
