...
//     value.entries.forEach((sector, index) => {
//       props.form.entries[index].voyage_expenditure_sub_head_id = sector?.voyage_expenditure_sub_head?.id ?? '';
//     });
//   }
// }, {deep: true});
...
<template>
  ...
  <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
    <label class="block w-full mt-3 text-sm md:w-2/6">
      ...
      <input type="text" readonly  :value="voyage?.voyage" placeholder="Voyage no" class="block w-full mt-1 text-sm rounded bg-gray-200 dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
  ...
  <fieldset class="px-4 pb-4 mt-3 border border-gray-700 rounded dark:border-gray-400">
    ...
    <table class="w-full whitespace-no-wrap" id="table">
      ...
      <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800" v-if="form?.entries?.length >= 1 && entryType == 'new'" v-for="(entry, index) in form.entries" :key="index">
      ...
      <tr v-if="form.entries[index]?.voyage_expenditure_head?.sub_head?.length > 0" v-for="(subhead, headIndex) in form.entries[index].voyage_expenditure_head.sub_head">
        ...
        <td class="px-1 py-1">
          <table class="w-full">
            <tr>
              <td class="" style="border: 0px">
                <input type="number" step=".01" v-model="subhead.amount" placeholder="Amount" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
              </td>
            </tr>
          </table>
        </td>
        ...
        <td class="px-1 py-1 text-center">
          <button v-if="subhead?.deletable == true" type="button" @click="removeExpenseRow(index, headIndex)" class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
            </svg>
          </button>
          <button v-else type="button" @click="addExpenseRow(index, headIndex)" class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
      ...
      <tr v-else>
        ...
        <td class="px-1 py-1 text-center">
          <button v-if="form.entries[index].deletable == true" type="button" @click="removeExpenseRow(index, 'head')" class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
            </svg>
          </button>
          <button v-else type="button" @click="addExpenseRow(index, 'head')" class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
              <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
            </svg>
          </button>
      ...
      <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800" v-if="form?.entries?.length >= 1 && entryType == 'entries'" v-for="(entry, index) in form.entries" :key="index">
      <tr class="text-gray-700 dark:text-gray-400">
        <td class="px-1 py-1" style="width: 20%" colspan="7">
          <input type="text" readonly v-model="form.entries[index].name" name="port_code" class="block w-full mt-1 text-sm rounded bg-green-200 dark:text-gray-300 dark:border-gray-600 dark:bg-green-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
        </td>
      </tr>
      <tr v-if="form.entries[index]?.entries.length > 0 && form.entries[index]?.ishead == 0" v-for="(subhead, headIndex) in form.entries[index].voyage_expenditure_head.sub_head">
        <td class="px-1 py-1" style="width: 20%">
          <input type="text" readonly :value="subhead?.name" name="port_code" class="block w-full mt-1 text-sm rounded bg-gray-200 dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
        </td>
        <td class="px-1 py-1">
