...
<template>
    <div @dragenter.prevent="toggleActive()" @dragleave.prevent="toggleActive()" @dragover.prevent @drop.prevent="drop" :class="{ 'bg-green-100 border-green-300 ': dragActive }" class="flex items-center justify-center w-full px-4 py-6 border-4 border-dashed rounded ">
        <div class="flex custom-dropzone flex-col items-center justify-center gap-2 text-gray-500 ">
            <span>
            ...
            <!-- File input -->
            <label for="file" class="p-2 text-sm font-semibold leading-tight text-gray-600 bg-purple-100 border rounded cursor-pointer  hover:bg-purple-200  hover:shadow-sm">
                <span class="text-purple-500 ">Select a file from your device</span>
                <input type="file" @change="selectedFile" name="file" id="file" class="hidden" multiple />
            </label>
            <!-- Dropped file info -->

          <div v-if="contractAttachments?.length" class="flex flex-col items-center justify-center space-x-1 text-base font-semibold text-gray-600 ">
            ...
            <div v-if="droppedFile.length" class="flex flex-col items-center justify-center space-x-1 text-base font-semibold text-gray-600 ">
                <div class="flex flex-wrap items-center justify-center">
                    ...
                    <span v-for="(singleFile,index) in droppedFile" class="flex ml-2">
                      <div>
                        {{ singleFile.name }}
                        <span class="text-xs text-gray-500  block">{{ bytes(singleFile.size) }}</span>
                      </div>
