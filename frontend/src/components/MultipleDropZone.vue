...
<template>
    <div @dragenter.prevent="toggleActive()" @dragleave.prevent="toggleActive()" @dragover.prevent @drop.prevent="drop" :class="{ 'bg-green-100 border-green-300 dark-disabled:bg-green-700': dragActive }" class="flex items-center justify-center w-full px-4 py-6 border-4 border-dashed rounded dark-disabled:border-2">
        <div class="flex custom-dropzone flex-col items-center justify-center gap-2 text-gray-500 dark-disabled:text-gray-100">
          ...
          <div v-if="contractAttachments?.length" class="flex flex-col items-center justify-center space-x-1 text-base font-semibold text-gray-600 dark-disabled:text-gray-100">
            ...
            <div v-if="droppedFile.length" class="flex flex-col items-center justify-center space-x-1 text-base font-semibold text-gray-600 dark-disabled:text-gray-100">
                <div class="flex flex-wrap items-center justify-center">
                    ...
                    <span v-for="(singleFile,index) in droppedFile" class="flex ml-2">
                      <div>
                        {{ singleFile.name }}
                        <span class="text-xs text-gray-500 dark-disabled:text-gray-300 block">{{ bytes(singleFile.size) }}</span>
                      </div>
