...
const { report, formParams, getProfitabilityReport, downloadProfitabilityReport, expenseHeads, expenseEntries } = useExpenditureReport();
...
<template>
  ...
  <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
    ...
    <label class="block w-full mt-3 text-sm md:w-3/6">
      <span class="text-gray-700 dark:text-gray-300">Round Voyage </span>
      <v-select v-model="formParams.voyagePair" :id="'voyagePair' + index" @search="fetchVoyagePairs" value="id" :options="pairList" :reduce="pairList => pairList.id" label="combined_name" placeholder="Search Round Voyage" class="block w-full mt-1 text-sm rounded dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input"></v-select>
    </label>
    ...
    <label class=" w-full mt-3 text-sm md:w-1/6 block">
      <button type="button" class="btn-submit-default md:mt-6" @click="tableToExcel('budget-vs-expense-report','Budget vs Expense Report')">Download</button>
    </label>
  </div>

  <div class="overflow-x-auto my-3 py-2 scroll-smooth scroll-show">
    <div v-html="report"></div>
    <table v-html="report" id="budget-vs-expense-report" class="hidden"></table>
  </div>
</template>
