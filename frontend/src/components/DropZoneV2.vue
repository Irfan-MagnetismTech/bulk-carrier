<template>
    <div @dragenter.prevent="toggleActive()" @dragleave.prevent="toggleActive()" style="height: auto" @dragover.prevent @drop.prevent="drop" :class="{ 'bg-green-100 border-green-300 ': dragActive }" class="flex items-center justify-center w-full px-4 py-6 border-4 border-dashed rounded ">
        <div class="flex custom-dropzone flex-col items-center justify-center gap-2 text-gray-500 ">
            ...
            <!-- Dropped file info -->
            <div v-if="droppedFile !== null" class="flex flex-col items-center justify-center space-x-1 text-base font-semibold text-gray-600 ">
                ...
                <span class="text-xs text-gray-500 ">{{ bytes(droppedFile?.size) }}</span>
            </div>
        </div>
    </div>
</template>
