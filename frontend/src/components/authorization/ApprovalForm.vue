...
<template>
    ...
    <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
        <label class="block w-full mt-3 text-sm md:w-2/6">
            ...
            <select v-model="form.model" required class="block w-full mt-1 text-sm rounded   focus:border-purple-400 focus:outline-none focus:shadow-outline-purple  form-input">
    ...
    <fieldset class="px-4 pb-4 mt-3 border border-gray-700 rounded ">
        <legend class="px-2 text-gray-700 ">Approval Bodies <span class="text-red-500">*</span></legend>
        <table class="w-full whitespace-no-wrap" id="table">
            ...
            <tbody class="bg-white divide-y ">
                <tr class="text-gray-700 " v-for="(approvalBody, index) in form.approval_bodies" :key="index">
                    <td class="px-1 py-1">
                        <table class="w-full">
                            <tr>
                                <td class="w-full" style="border: 0px">
                                    <input type="number" v-model="form.approval_bodies[index].approval_order" placeholder="ex: 1" required class="block w-full rounded xt-sm 1   focus:border-purple-400 focus:outline-none focus:shadow-outline-purple  form-input">
                                </td>
                            </tr>
                        </table>
                    </td>
                    <td class="px-1 py-1" style="width: 40%">
                        <select v-model="form.approval_bodies[index].approver_id" name="approver_id" required class="block w-full rounded xt-sm 1   focus:border-purple-400 focus:outline-none focus:shadow-outline-purple  form-input">
                            <option value="">Select Option</option>
                          <option :value="user?.id" v-for="(user,index) in approvalSubjectUser?.users">{{user?.name}}</option>
                        </select>
                    </td>
                  <td class="px-1 py-1">
                    <select v-model="form.approval_bodies[index].approver_role" name="approver_role" required class="block w-full rounded xt-sm 1   focus:border-purple-400 focus:outline-none focus:shadow-outline-purple  form-input">
                      <option value="">Select Option</option>
