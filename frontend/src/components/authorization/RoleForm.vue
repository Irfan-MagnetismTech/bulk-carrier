...
<template>
  ...
  <div class="flex flex-col justify-center w-full md:flex-row md:gap-2">
    <label class="block w-full mt-3 text-sm">
      <span class="text-gray-700 dark-disabled:text-gray-300">Role Name <span class="text-red-500">*</span></span>
      <input type="text" v-model="form.name" required placeholder="Role Name" class="block w-full mt-1 text-sm rounded dark-disabled:text-gray-300 dark-disabled:border-gray-600 dark-disabled:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray form-input" />
    </label>
  </div>
  <fieldset class="px-4 pb-4 mt-3 border border-gray-700 rounded dark-disabled:border-gray-400">
    <legend class="px-2 text-gray-700 dark-disabled:text-gray-300">Permissions <span class="text-red-500">*</span></legend>
    <div class="mt-2">
      <div :id="'menu_' + menuIndex" :class="menuIndex%2===0 ? 'bg-gray-100' : 'bg-yellow-100'" style="position: relative" class="px-2 py-2 border sm:rounded-lg mb-1" v-for="(permissionMenuData,permissionMenuKey,menuIndex) in form?.permissions" :key="menuIndex">
        <label class="flex items-center mb-2 text-gray-600 dark-disabled:text-gray-400">
          <input @change="inputCheckBymenu2(permissionMenuKey)" v-model="form.permissions[permissionMenuKey].is_checked" type="checkbox" class="text-purple-600 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray">
        ...
        <template v-for="(permissionSubjectData,permissionSubjectKey,subjectIndex) in form?.permissions[permissionMenuKey]" :key="subjectIndex">
          <label class="flex ml-6 items-center mb-2 text-gray-600 dark-disabled:text-gray-400" v-if="permissionSubjectKey!=='is_checked'">
            <input @change="inputCheckedBySubject2(permissionMenuKey,permissionSubjectKey)" type="checkbox" :class="'menu_' + menuIndex" v-model="form.permissions[permissionMenuKey][permissionSubjectKey].is_checked" class="text-purple-600 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray">
          ...
          <div class="inline-block mb-2 ml-12">
            <template v-for="(permissionData,permissionIndex) in form?.permissions[permissionMenuKey][permissionSubjectKey]" :key="permissionIndex">
              <label @change="inputCheckedByPermission(permissionMenuKey,permissionSubjectKey,permissionIndex)" v-if="permissionData?.name" class="flex flex-column items-center text-gray-600 dark-disabled:text-gray-400 ml-2 break-words">
                <input type="checkbox" :class="'menu_' + menuIndex,'permission_' + subjectIndex" class="text-purple-600 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray" v-model="form.permissions[permissionMenuKey][permissionSubjectKey][permissionIndex].is_checked" :value="permissionData?.id">
