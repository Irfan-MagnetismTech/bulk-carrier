...
import useMaterial from "../../../../composables/supply-chain/useMaterial";
    ...
    const materialChange = () => {
        props.form.scm_material_id = props.form.scmMaterial?.id;
    }
    ...
    onMounted(() => {
        ...
        searchMaterial('', businessUnit.value);
    });
...
<template>
    <div class="justify-center w-full grid grid-cols-1 md:grid-cols-3 md:gap-2">
        ...
        <label class="block w-full mt-2 text-sm">
            <span class="text-gray-700 dark-disabled:text-gray-300">Warehouse Name <span class="text-red-500">*</span></span>
        ...
        <label class="block w-full mt-2 text-sm">
            ...
            <v-select placeholder="--Choose an option--" :loading="isMaterialLoading"  :options="materials" @search="" v-model="form.scmMaterial" label="material_name_and_code" @update:modelValue="materialChange"  class="block form-input">
                <template #search="{attributes, events}">
                    <input
                                class="vs__search"
                                :required="!form.scmMaterial"
                                v-bind="attributes"
                                v-on="events"
                            />
                </template>
            </v-select>
            <input type="hidden" v-model="form.scm_material_id">
        </label>
