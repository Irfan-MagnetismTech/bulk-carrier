<template>
  <div class="input-group">
    <label class="label-group">
      <span class="label-item-title font-bold"
        >CS Ref<span class="text-red-500"> : </span></span
      >
      <span class="label-item-title"> {{ form.ref_no }}</span>
    </label>
    <label class="label-group">
      <span class="label-item-title font-bold"
        >Warehouse<span class="text-red-500"> : </span></span
      >
      <span class="label-item-title"> {{ form.scmWarehouse?.name }}</span>
    </label>
    <label class="label-group">
      <span class="label-item-title font-bold"
        >Expire Date<span class="text-red-500"> : </span></span
      >
  ...
  <div class="input-group">
    <label class="label-group">
      <span class="label-item-title font-bold"
        >Requirement Type<span class="text-red-500"> : </span></span
      >
      <span class="label-item-title"> {{ form.priority }}</span>
    </label>
    <label class="label-group">
      <span class="label-item-title font-bold"
        >Required Days<span class="text-red-500"> : </span></span
      >
    ...
    <label class="label-group">
      <span class="label-item-title font-bold"
        >Effective Date<span class="text-red-500"> : </span></span
      >
      <span class="label-item-title">
        {{ formatDate(form.effective_date) }}</span
      >
    </label>
  </div>
  ...
  <div class="input-group">
    <label class="label-group">
      <span class="label-item-title font-bold"
        >Special Instruction<span class="text-red-500"> : </span></span
      >
  ...
  <table
    class="min-w-full divide-y divide-gray-200 dark-disabled:divide-gray-700 mt-10"
  >
    <caption class="border-2 bg-green-400 uppercase">
      Comparative Statement
    </caption>
    <thead class="bg-gray-50 dark-disabled:bg-gray-800">
      <tr
        class="text-gray-600 dark-disabled:text-gray-400 text-sm leading-normal"
      >
        <th
          rowspan="2"
          class="px-6 py-3 text-left text-gray-600 dark-disabled:text-gray-400 uppercase tracking-wider"
        >
       ...
       <nobr>PR No</nobr> 
        ...
        <th
          ...
          scope="col"
          class="px-6 py-3 text-left text-gray-600 dark-disabled:text-gray-400 uppercase tracking-wider"
          v-for="(vendorData, index) in formData?.scmCsVendor"
          :key="index"
        >
    ...
    <tbody>
      <template
        v-for="(materialprData, index1) in formData?.scmCsMaterial"
        :key="index1"
      >
        <template
          v-for="(materialData, name, index) in materialprData"
          :key="index"
        >
          <tr v-if="index == 0">
            <td :rowspan="Object.keys(materialprData).length">
              {{ materialData[0].scmMaterial.name }}
            </td>
            ...
            <td>
              {{
                formData?.latestPoItem && formData?.latestPoItem.length
                  ? formData?.latestPoItem[index1].rate
                  : "N/A"
              }}
            </td>
            <template
              v-for="(materialVendorData, index11) in formData
                ?.scmCsMaterialVendor[index1][name]"
              :key="index11"
            >
              <td>{{ materialVendorData[0].negotiated_price }}</td>
              <td>
                {{
                  materialVendorData[0].negotiated_price *
                  materialData[0].sum_quantity
                }}
              </td>
            </template>
            <td :rowspan="Object.keys(materialprData).length">
              <a
                @click="showModal(index1, name)"
  ...
  >
    ...
    <thead class="bg-gray-50 dark-disabled:bg-gray-800">
      ...
      >
        <th
          class="px-6 py-3 text-center text-gray-600 dark-disabled:text-gray-400 uppercase tracking-wider"
        >
          Description
        </th>
        <th
    ...
    <tbody>
      ...
      <tr v-if="form.purchase_center == 'Foreign'">
        <td>Offer Price</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].total_offered_price }}
        </td>
      </tr>
      <tr v-if="form.purchase_center == 'Foreign'">
        <td>Freight</td>
      ...
      <tr v-if="form.purchase_center == 'Foreign'">
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].grand_total_offered_price }}
        </td>
      </tr>
      <tr v-if="form.purchase_center == 'Foreign'">
        <td>Negotiated Price</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].total_negotiated_price }}
        </td>
      </tr>
      ...
      <tr>
        <td>Payment Term</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].credit_term }}
        </td>
      </tr>
      <tr>
        <td>Delivery Time</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].delivery_time }}
        </td>
      </tr>
      <tr v-if="form.purchase_center == 'Foreign'">
        <td>Port Of Discharge</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].port_of_discharge }}
        </td>
      </tr>
      <tr v-if="form.purchase_center == 'Foreign'">
        <td>Mode Of Shipment</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].mode_of_shipment }}
        </td>
      </tr>
      <tr>
        <td>Offer Validity</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].quotation_validity }}
        </td>
      </tr>
      <tr>
        <td>Warranty</td>
        <template
          v-for="(VendoData, index) in formData?.scmCsVendor"
          :key="index"
        >
          <td>
            {{
              details[index] &&
              details[index][0] &&
              details[index][0].warranty_period
            }}
          </td>
        </template>
      </tr>
      <tr>
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].sourcing }}
        </td>
      </tr>
      <tr>
        <td>Party Type</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].scmVendor.product_source_type }}
        </td>
      </tr>
      <tr>
        <td>Inventory Type</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].stock_type }}
        </td>
      </tr>
      <tr>
        <td>Manufacturing Days</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].manufacturing_days }}
        </td>
      </tr>
      ...
      <tr>
        <td>Quotation Ref</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].quotation_ref }}
        </td>
      </tr>
      <tr>
        <td>Quotation Received Date</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ formatDate(VendoData[0].quotation_received_date) }}
        </td>
      </tr>
      ...
      <tr>
        <td>Date Of RFQ</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ formatDate(VendoData[0].date_of_rfq) }}
        </td>
      </tr>
      ...
      <tr v-if="form.purchase_center == 'Foreign'">
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ formatDate(VendoData[0].quotation_shipment_date) }}
        </td>
      </tr>
      <tr v-if="form.purchase_center == 'Foreign'">
        <td>Port Of Loading</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].port_of_loading }}
        </td>
      </tr>
      ...
      <tr>
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].terms_and_condition }}
        </td>
      </tr>
      <tr v-if="form.purchase_center != 'Foreign'">
        <td>VAT</td>
      ...
      <tr v-if="form.purchase_center != 'Foreign'">
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].carring_cost_bear_by }}
        </td>
      </tr>
      <tr v-if="form.purchase_center != 'Foreign'">
        <td>Unloading Charge Bear By</td>
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          {{ VendoData[0].unloading_cost_bear_by }}
        </td>
      </tr>
      ...
      <tr>
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          <select
            v-model="form.scmCsVendor[index][0].technical_acceptance"
            class="form-input"
          >
      ...
      <tr>
        ...
        <td v-for="(VendoData, index) in formData?.scmCsVendor" :key="index">
          <input
            type="checkbox"
            :class="'menu_' + VendoData.scm_cs_id"
            class="text-purple-600 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark-disabled:focus:shadow-outline-gray"
            :value="VendoData?.id"
            v-model="form.scmCsVendor[index][0].is_selected"
          />
        </td>
      </tr>
    </tbody>
  ...
  <div class="input-group">
    <label class="label-group">
      <span class="label-item-title"
        >Selection Ground<span class="text-red-500">*</span></span
      >
      <v-select
        :options="selection_ground"
        placeholder="--Choose an option--"
        v-model="form.selection_ground"
        label="name"
        class="block form-input"
        :reduce="(selection_ground) => selection_ground.value"
      >
    ...
    <label class="label-group">
      <span class="label-item-title"
        >Auditor Remarks Date<span class="text-red-500">*</span></span
      >
      <VueDatePicker
  ...
  <div
    v-show="isModalOpen"
    class="fixed inset-0 z-30 flex items-end overflow-y-auto bg-black bg-opacity-50 sm:items-center sm:justify-center"
  >
    <!-- Modal -->
    <form @submit.prevent="" style="position: absolute; top: 0">
      <div
        class="w-full px-6 py-4 overflow-y-auto bg-white rounded-t-lg dark-disabled:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl"
        role="dialog"
        id="modal"
      >
        ...
        <!-- Modal body -->
        <table
          class="w-full mb-2 whitespace-no-wrap border-collapse contract-assign-table table2"
        >
          <thead v-once>
            <tr
        ...
        <div class="dt-responsive table-responsive">
          <table
            id="dataTable"
            class="w-full table table-striped table-bordered"
          >
            <thead>
              <tr>
                <th>Description</th>
                ...
                >
                  <td>
                    {{
                      details[index] &&
                      details[index][0] &&
                      details[index][0].installation_and_commission
                    }}
                  </td>
                </template>
              </tr>
                ...
                >
                  <td>
                    {{
                      details[index] &&
                      details[index][0] &&
                      details[index][0]?.negotiated_price
                    }}
                  </td>
                </template>
              </tr>
        ...
        <footer
          class="flex flex-col items-center justify-end px-6 py-3 -mx-6 -mb-4 space-y-4 sm:space-y-0 sm:space-x-6 sm:flex-row bg-gray-50 dark-disabled:bg-gray-800"
        >
          <button
            type="button"
            @click="closeModel"
...
const { warehouses, warehouse, getWarehouses, searchWarehouse } =
  useWarehouse();
...
function showModal(index1, name) {
  ...
  if (props.formData?.scmCsMaterialVendor[index1][name]) {
    details.value = cloneDeep(
      props.formData?.scmCsMaterialVendor[index1][name]
    );
  } else {
    details.value = [];
  }
