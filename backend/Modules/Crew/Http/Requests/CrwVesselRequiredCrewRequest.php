...
class CrwVesselRequiredCrewRequest extends FormRequest
{
    ...
    public function rules(): array {
        $id = $this->route('crw_vessel_required_crew');

        return [
            'total_crew'                                     => ['required', 'numeric', 'max:2000'],
            'effective_date'                                 => ['required', Rule::unique('crw_vessel_required_crews')->where('effective_date', $this->effective_date)->ignore($id)],
